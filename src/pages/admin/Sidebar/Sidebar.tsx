...
const SidebarComponent = ({ isOpen, toggleSidebar, setIsOpen }: { isOpen: any, toggleSidebar: any, setIsOpen: React.Dispatch<React.SetStateAction<boolean>> }) => {
  ...
  return (
    <div className={`fixed left-0 top-0 h-full  from-[#1a251f] via-[#1d1b20] to-[#231a27] text-white transition-all flex flex-col duration-300
      ...
      <div className="mt-6 flex-1">
        {
          menuItems.map((item, index) => (
            <MenuItem
              ...
              onClicked={() => { navigate("/admin/" + item.key) }}
