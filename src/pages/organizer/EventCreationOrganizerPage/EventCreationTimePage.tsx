...
const EventCreationTimePage: React.FC<EventCreationTimePageProps> = ({
  dataEventCreate,
  setDataEventCreate,
  isEdit,
  idEvent,
      ...
      if (updatedPerformances[index]) {
        ...
        if (field === 'endTime') {
          if (updatedPerformances[index].startDate < new Date()) {
            ...
            if (
              new Date(updatedPerformances[index].startDate) > new Date(value)
            ) {
              setErrorMessage((prev) => {
                const newErrors = [...prev];
                newErrors[index] =
          ...
          if (new Date(value) < new Date()) {
            ...
            if (
              new Date(updatedPerformances[index].endDate) < new Date(value)
            ) {
              setErrorMessage((prev) => {
                const newErrors = [...prev];
                newErrors[index] =
  ...
  const deletePerformance = (index: number, isData: boolean) => {
    // if (performances[index]) {
    //     setPerformances(prev => prev.filter((_, i) => i !== index));
    //
    if (isData) {
      setDataEventCreate((prev) => {
        const updatedPerformances = prev.showTimes.map((item, i) => {
          if (i === index) {
            const cloneItem = { ...item };
            cloneItem.status = 'Canceled';
            cloneItem.typeTickets = cloneItem.typeTickets.map((item) => {
      ...
      setDataEventCreate((prev) => {
        const updatedPerformances = prev.showTimes.filter(
          (_, i) => i !== index
        );
        return {
          ...prev,
  ...
  const handleUpdateTicket = (
    ...
    typeTicket: TypeTicketModel,
    indexShowTime: number,
    indexTypeTicket: number
  ) => {
    setModalState((prev) => {
      return {
        ...prev,
  ...
  const handleDeleteTicket = (
    ...
    if (isData) {
      setDataEventCreate((prev) => {
        const updatedPerformances = [...prev.showTimes];
        updatedPerformances[indexShowTime].typeTickets = updatedPerformances[
          indexShowTime
        ].typeTickets.map((item, i) => {
          if (i === indexTypeTicket) {
            return { ...item, status: 'Canceled' };
          }
          return item;
        });
      ...
      setDataEventCreate((prev) => {
        ...
        updatedPerformances[indexShowTime].typeTickets = updatedPerformances[
          indexShowTime
        ].typeTickets.filter((_, i) => i !== indexTypeTicket);
        return {
          ...prev,
          showTimes: updatedPerformances,
        };
  ...
  const renderTypeTicket = (
    ...
    return (
      <div
        className="bg-gray-500 rounded-md p-3 mt-2 mb-2 flex justify-between"
        key={typeTicket.name}
      >
        <RowComponent>
          <FaTicketAlt size={20} />
  ...
  const handSubmitShowTime = async (showTime: ShowTimeModel) => {
    if (modalShowTimeState.type === 'create') {
      if (isEdit) {
        ...
        try {
          setIsLoading(true);
          const res = await showTimeAPI.HandleShowTime(
            api,
            {
          ...
          toast.error(
            errorMessage.message ?? 'Thêm suất diễn không thành công'
          );
        }
      } else {
        setDataEventCreate((prev) => {
      ...
      if (isEdit) {
        ...
        try {
          ...
          const res = await showTimeAPI.HandleShowTime(
            api,
            { ...showTime, idShowTime: showTime._id, idEvent: idEvent },
            'put'
          );
          if (res && res.status === 200 && res.data) {
            setDataEventCreate((prev) => {
              const updateShowTime = [...prev.showTimes];
              updateShowTime[modalShowTimeState.indexShowTimeSelected] =
                showTime;
              return {
                ...prev,
          ...
          toast.error(
            errorMessage.message ?? 'Cập nhập suất diễn không thành công'
          );
        }
      } else {
        setDataEventCreate((prev) => {
    ...
    if (modalState.type === 'create') {
      if (isEdit) {
        ...
        try {
          ...
          const res = await typeTicketAPI.HandleTypeTicket(
            api,
            {
              ...val,
              idEvent: idEvent,
              idShowTime: modalState.showTimeSelected._id,
            },
            'post'
          );
          if (res && res.data && res.status === 200) {
            ...
            setDataEventCreate((prev) => {
              ...
              const updatedTypeTickets = [
                ...selectedPerformance.typeTickets,
                val,
              ];
          ...
          toast.error(
            errorMessage.message ?? 'Thêm loại loại vé không thành công'
          );
        }
      } else {
        setDataEventCreate((prev) => {
      ...
      if (isEdit) {
        ...
        try {
          ...
          if (res && res.data && res.status === 200) {
            ...
            setDataEventCreate((prev) => {
              const updatedPerformances = [...prev.showTimes];
              updatedPerformances[modalState.indexShowTimeSelected].typeTickets[
                modalState.indexTypeTicketSelected
              ] = val;
              return {
                ...prev,
                showTimes: updatedPerformances,
          ...
          toast.error(
            errorMessage.message ?? 'Cập nhập loại vé không thành công'
          );
        }
      } else {
        setDataEventCreate((prev) => {
    ...
    if (deleteState.content === 'showTime') {
      if (isEdit) {
        ...
        try {
          ...
          const res = await showTimeAPI.HandleShowTime(
            api,
            { idShowTime: deleteState.showTimeDelete._id, idEvent: idEvent },
            'delete'
          );
          if (res && res.status === 200) {
            deletePerformance(
              deleteState.indexShowTimeSelected,
              res.data ? true : false
            );
            toast.success(
              res.data
                ? 'Hủy suất diễn thành công'
        ...
        deletePerformance(deleteState.indexShowTimeSelected, false);
      ...
      if (isEdit) {
        ...
        try {
          ...
          const res = await typeTicketAPI.HandleTypeTicket(
            ...
            {
              idEvent: idEvent,
              idTypeTicket: deleteState.typeTicketDelete._id,
              idShowTime: deleteState.showTimeDelete._id,
            },
            'delete'
          );
          if (res && res.status === 200) {
            toast.success(
              res.data
                ? 'Hủy loại vé thành công !!!'
                : 'Xóa loại vé thành công !!!'
            );
            handleDeleteTicket(
              deleteState.indexShowTimeSelected,
              deleteState.indexTypeTicketSelected,
              res.data ? true : false
            );
          }
          setIsLoading(false);
            ...
            <div>
              ...
              <section className="flex flex-row gap-4">
                <div className="flex-1">
                  <label className="text-black dark:text-gray-400 text-sm font-semibold">
                    Thời gian bắt đầu
                  </label>
                  <input
                    disabled
              ...
              <div className="flex flex-col mt-4">
                ...
                </div>
                {
                  ...
                  >
                    {performance.typeTickets.map(
                      (typeTicket, indexTypeTicket) => {
                        return renderTypeTicket(
                          performance,
                          typeTicket,
                          indexTypeTicket,
                          indexShowTime
