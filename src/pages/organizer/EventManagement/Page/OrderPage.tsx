...
import {
  InvoiceDetailsModel,
  TicketsPurchase,
} from '../../../../models/InvoiceDetailsModel';
...
const OrderPage = ({ variants }: { variants: any }) => {
  ...
  const handleCallAPIGetAllInvoices = async () => {
    const api = apis.ticket.getByIdShowTime({
  ...
  return (
    <div className="min-h-screen bg-custom-gradient text-black p-6">
      ...
      <motion.div
        variants={variants}
        className="flex flex-col p-4 bg-white rounded-xl mt-12"
      >
        {/* Page Title */}
        ...
        <div className="mb-6">
          <div className="flex space-x-4 mb-4">
            ...
            <h3 className="text-lg font-semibold text-[20px] text-black mt-3">
              Suất diễn:
            </h3>
            ...
            <div className="w-1/3">
              ...
              >
                {allShowTime.map((showTime) => {
                  return (
                    <option value={showTime._id}>{`${DateTime.GetTime(
                      showTime.startDate
                    )} : ${DateTime.GetTime(
                      showTime.endDate
        ...
        <div>
          <h3 className="text-lg font-semibold mb-3 text-[20px]">
            Danh sách đơn hàng
          </h3>
              ...
              <tbody>
                {invoices &&
                  ...
                  invoices.map((invoice, index) => (
                    <tr
                      key={index}
                      className="border-b border-slate-200 text-[19px] "
                    >
                      <td className="px-1 py-3 text-center">
                        {invoice.invoiceDetails.invoiceCode}
                      </td>
                      <td className="px-1 py-3 text-center">
                        {invoice.invoiceDetails.fullname}
                      </td>
                      <td className="px-1 py-3 text-center">
                        {invoice.invoiceDetails.email}
                      </td>
                      <td className="px-1 py-3 text-center">
                        {invoice.invoiceDetails.phoneNumber}
                      </td>
                      <td className="px-1 py-3 text-center">{`${DateTime.GetTime(
                        invoice.invoiceDetails.createdAt
                      )} ${DateTime.GetDate2(
                        invoice.invoiceDetails.createdAt
                      )}`}</td>
                      <td className="px-1 py-3 text-center">
                      ...
                      <td className="px-4 py-3 text-center">
                        {convertMoney(invoice.invoiceDetails.totalPrice ?? 0)}
                      </td>
                      <td className="px-4 py-3 text-center">
